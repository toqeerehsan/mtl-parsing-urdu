...
train = _parse_data(archive.open('mtl-data/train.mtl'))
test = _parse_data(archive.open('mtl-data/test.mtl'))
...
input_0 = Input(shape=(MAX_LENGTH, elmo_dim),dtype='float32')
...
if include_w2v_emb=="yes":
    merged_1 = concatenate([input_0 , emb_1])
else:
    merged_1 = input_0
...
if include_w2v_emb == 'yes':
    model = Model(inputs=[input_0, input_1, input_2], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
else:
    model = Model(inputs=[input_0, input_2], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
...
#model.save("trained_models/mtl-blstm-fun-pos-elmo+w2v-24.model")

print('Loading pretrained Labeling model ...')
model = load_model("trained_models/mtl-blstm-pos-elmo-21.model")
#
...
if include_w2v_emb == 'yes':
    test_y_lu_pred = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[0].argmax(-1)[test_sentences_X > 2]
    test_y_seq_pred = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[3].argmax(-1)[test_sentences_X > 2]
    test_y_dep_pred = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[4].argmax(-1)[test_sentences_X > 2]
else:
    test_y_lu_pred = model.predict([elmo_test_x,test_tags_x])[0].argmax(-1)[test_sentences_X > 2]
    test_y_seq_pred = model.predict([elmo_test_x,test_tags_x])[3].argmax(-1)[test_sentences_X > 2]
    test_y_dep_pred = model.predict([elmo_test_x,test_tags_x])[4].argmax(-1)[test_sentences_X > 2]
...
if include_w2v_emb == 'yes':
    predictions_lu = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[0]
    predictions_seq = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[3]
    predictions_dep = model.predict([elmo_test_x,test_sentences_X, test_tags_x])[4]
else:
    predictions_lu = model.predict([elmo_test_x, test_tags_x])[0]
    predictions_seq = model.predict([elmo_test_x, test_tags_x])[3]
    predictions_dep = model.predict([elmo_test_x, test_tags_x])[4]
