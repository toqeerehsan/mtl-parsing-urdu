...
dense_out_lu = TimeDistributed(Dense(len(lu2index),activation='softmax'))(merged_5)
dense_out_seq = TimeDistributed(Dense(len(seq2index),activation='softmax'))(merged_5)
...
if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        history = model.fit([train_sentences_X, train_chars_X, train_tags_x], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index))], batch_size=32, epochs=18, validation_data=([dev_sentences_X,dev_chars_X,dev_tags_x], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index))]), shuffle = True)
    else:
        history = model.fit([train_sentences_X, train_chars_X], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index))], batch_size=32, epochs=18, validation_data=([dev_sentences_X, dev_chars_X], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index))]), shuffle=True)
else:
    if include_pos_emb == 'yes':
        history = model.fit([train_sentences_X,train_tags_x], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index))], batch_size=32, epochs=18, validation_data=([dev_sentences_X,dev_tags_x], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index))]), shuffle = True)
    else:
        history = model.fit([train_sentences_X], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index))], batch_size=32, epochs=18, validation_data=([dev_sentences_X], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index))]), shuffle=True)
"""
...
#model.save("trained_models/3blstm-chemb-pos-luseq2.model")

print('Loading pretrained Labeling model ...')
model = load_model("trained_models/3blstm-chemb-luseq-80.25.model")
#
...
if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        ...
        test_y_seq_pred = model.predict([test_sentences_X, test_chars_X, test_tags_x])[1].argmax(-1)[test_sentences_X > 2]
    else:
        test_y_lu_pred = model.predict([test_sentences_X, test_chars_X])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X, test_chars_X])[1].argmax(-1)[test_sentences_X > 2]
else:
    if include_pos_emb == 'yes':
        test_y_lu_pred = model.predict([test_sentences_X, test_tags_x])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X, test_tags_x])[1].argmax(-1)[test_sentences_X > 2]
    else:
        test_y_lu_pred = model.predict([test_sentences_X])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X])[1].argmax(-1)[test_sentences_X > 2]
...
if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        ...
        predictions_seq = model.predict([test_sentences_X, test_chars_X, test_tags_x])[1]
    else:
        predictions_lu = model.predict([test_sentences_X, test_chars_X])[0]
        predictions_seq = model.predict([test_sentences_X, test_chars_X])[1]
else:
    if include_pos_emb == 'yes':
        predictions_lu = model.predict([test_sentences_X, test_tags_x])[0]
        predictions_seq = model.predict([test_sentences_X, test_tags_x])[1]
    else:
        predictions_lu = model.predict([test_sentences_X])[0]
        predictions_seq = model.predict([test_sentences_X])[1]
