...
print(seq_tags)
dep_tags = sorted(list(set(row[4] for sample in train + test + dev for row in sample if row[4] not in ['-BOS-@-BOS-{}-BOS-','-EOS-@-EOS-{}-EOS-'])))  # in alphabetic order
dep_tags = ['-PAD-','-BOS-@-BOS-{}-BOS-','-EOS-@-EOS-{}-EOS-']+dep_tags
print(dep_tags)
...
print("Total Dep Labels: ",len(dep_tags))
...
dep2index = {t: i for i, t in enumerate(list(dep_tags))}

# train and test sentences to have number rather than words and tags for training
train_sentences_X, test_sentences_X, dev_sentences_X, train_tags_x, test_tags_x, dev_tags_x, train_lu_y, test_lu_y, dev_lu_y, train_seq_y, test_seq_y, dev_seq_y, train_dep_y, test_dep_y, dev_dep_y = [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
...
for s in train_dep_tags:
    train_dep_y.append([dep2index[t] for t in s.split()])
for s in test_dep_tags:
    test_dep_y.append([dep2index[t] for t in s.split()])
for s in dev_dep_tags:
    dev_dep_y.append([dep2index[t] for t in s.split()])
...
train_dep_y = pad_sequences(train_dep_y, maxlen=MAX_LENGTH, padding='post')
test_dep_y = pad_sequences(test_dep_y, maxlen=MAX_LENGTH, padding='post')
dev_dep_y = pad_sequences(dev_dep_y, maxlen=MAX_LENGTH, padding='post')
...
if include_w2v_emb=='yes':
    ...
    emb_1= Embedding(input_dim=len(vocab), output_dim=100, input_length=MAX_LENGTH)(input_1)
...
merged_2 = Dropout(0.25)(merged_2)
...
out_lu = TimeDistributed(Dense(len(lu2index),activation='softmax'))(merged_4)
out_seq = TimeDistributed(Dense(len(seq2index),activation='softmax'))(merged_4)
out_dep = TimeDistributed(Dense(len(dep2index),activation='softmax'))(merged_4)

seq_LSTM3= Bidirectional(LSTM(256, return_sequences=True, recurrent_dropout=0.5))(concatenate([merged_4, out_lu, out_dep]))
merged_6 = Dropout(0.25)(seq_LSTM3)
out_seq2 = TimeDistributed(Dense(len(seq2index),activation='softmax'))(merged_6)

dep_LSTM3= Bidirectional(LSTM(256, return_sequences=True, recurrent_dropout=0.5))(concatenate([merged_4, out_seq,out_lu]))
merged_6 = Dropout(0.25)(dep_LSTM3)
out_dep2 = TimeDistributed(Dense(len(dep2index),activation='softmax'))(merged_6)

model = Sequential()

if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        model = Model(inputs=[input_1, char_in, input_2], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
    else:
        model = Model(inputs=[input_1, char_in], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
else:
    if include_pos_emb == 'yes':
        model = Model(inputs=[input_1, input_2], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
    else:
        model = Model(inputs=[input_1], outputs=[out_lu,out_seq,out_dep,out_seq2,out_dep2])
...
if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        history = model.fit([train_sentences_X, train_chars_X, train_tags_x], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index))], batch_size=32, epochs=24, validation_data=([dev_sentences_X,dev_chars_X,dev_tags_x], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index))]), shuffle = True)
    else:
        history = model.fit([train_sentences_X, train_chars_X], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index))], batch_size=32, epochs=24, validation_data=([dev_sentences_X, dev_chars_X], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index))]), shuffle=True)
else:
    if include_pos_emb == 'yes':
        history = model.fit([train_sentences_X,train_tags_x], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index))], batch_size=32, epochs=24, validation_data=([dev_sentences_X,dev_tags_x], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index))]), shuffle = True)
    else:
        history = model.fit([train_sentences_X], [to_categorical(train_lu_y, len(lu2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index)),to_categorical(train_seq_y, len(seq2index)),to_categorical(train_dep_y, len(dep2index))], batch_size=32, epochs=24, validation_data=([dev_sentences_X], [to_categorical(dev_lu_y, len(lu2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index)),to_categorical(dev_seq_y, len(seq2index)),to_categorical(dev_dep_y, len(dep2index))]), shuffle=True)
...
model.save("trained_models/mtl-blstm-fun-chemb-pos-w2v-24.model")

#print('Loading pretrained Labeling model ...')
#model = load_model("trained_models/mtl-blstm-fun-chemb-24.model")
#
...
if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        test_y_lu_pred = model.predict([test_sentences_X,test_chars_X, test_tags_x])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X, test_chars_X, test_tags_x])[3].argmax(-1)[test_sentences_X > 2]
        test_y_dep_pred = model.predict([test_sentences_X, test_chars_X, test_tags_x])[4].argmax(-1)[test_sentences_X > 2]
    else:
        test_y_lu_pred = model.predict([test_sentences_X, test_chars_X])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X, test_chars_X])[3].argmax(-1)[test_sentences_X > 2]
        test_y_dep_pred = model.predict([test_sentences_X, test_chars_X])[4].argmax(-1)[test_sentences_X > 2]
else:
    if include_pos_emb == 'yes':
        test_y_lu_pred = model.predict([test_sentences_X, test_tags_x])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X, test_tags_x])[3].argmax(-1)[test_sentences_X > 2]
        test_y_dep_pred = model.predict([test_sentences_X, test_tags_x])[4].argmax(-1)[test_sentences_X > 2]
    else:
        test_y_lu_pred = model.predict([test_sentences_X])[0].argmax(-1)[test_sentences_X > 2]
        test_y_seq_pred = model.predict([test_sentences_X])[3].argmax(-1)[test_sentences_X > 2]
        test_y_dep_pred = model.predict([test_sentences_X])[4].argmax(-1)[test_sentences_X > 2]
#test_y_pred = model.predict([test_sentences_X,test_chars_X]).argmax(-1)[test_sentences_X > 2]

test_y_lu_true = test_lu_y[test_sentences_X > 2]
test_y_seq_true = test_seq_y[test_sentences_X > 2]
test_y_dep_true = test_dep_y[test_sentences_X > 2]

if include_char_emb=='yes':
    if include_pos_emb == 'yes':
        predictions_lu = model.predict([test_sentences_X,test_chars_X, test_tags_x])[0]
        predictions_seq = model.predict([test_sentences_X, test_chars_X, test_tags_x])[3]
        predictions_dep = model.predict([test_sentences_X, test_chars_X, test_tags_x])[4]
    else:
        predictions_lu = model.predict([test_sentences_X, test_chars_X])[0]
        predictions_seq = model.predict([test_sentences_X, test_chars_X])[3]
        predictions_dep = model.predict([test_sentences_X, test_chars_X])[4]
else:
    if include_pos_emb == 'yes':
        predictions_lu = model.predict([test_sentences_X, test_tags_x])[0]
        predictions_seq = model.predict([test_sentences_X, test_tags_x])[3]
        predictions_dep = model.predict([test_sentences_X, test_tags_x])[4]
    else:
        predictions_lu = model.predict([test_sentences_X])[0]
        predictions_seq = model.predict([test_sentences_X])[3]
        predictions_dep = model.predict([test_sentences_X])[4]
...
results_dep = logits_to_tokens(predictions_dep, {i: t for t, i in dep2index.items()})

print('\n---- Result of BiLSTM-Sequence Labeling ----\n')
classification_report(test_y_dep_true, test_y_dep_pred, dep_tags)
...
out_file_dep = open("trained_models/outputs/output.dep",'w')
...
for i,line in enumerate(test_sents):
    ...
    for j,w in enumerate(line):
        out_file_dep.write(w + "\t" + pos_line[j] + "\t" + results_dep[i][j] + "\n")
...
os.system("./EVALB/evalb trained_models/outputs/gold-fun.txt trained_models/outputs/output.brackets -p EVALB/new.prm")
#os.system("discodop eval trained_models/outputs/gold.txt trained_models/outputs/output.brackets EVALB/COLLINS.prm --goldfmt=bracket --parsesfmt=bracket")
...
os.system("python dep2label-master/decode_labels2dep.py --input trained_models/outputs/output.dep  --output trained_models/outputs/output.conll --encoding rel-pos")
os.system("perl trained_models/outputs/eval-spmrl.pl -q -g trained_models/outputs/test_ref.conll -s trained_models/outputs/output.conll")
